...
std::list<libcomp::Message::MessageType> LobbyManager::GetSupportedTypes()
    const {
  return {
...
bool LobbyManager::HandlePacketLobbyCharacterList(libcomp::ReadOnlyPacket &p) {
  ...
  bool changed = !mCharacterList ||
                 obj->CharactersCount() != mCharacterList->CharactersCount();
  ...
  if (changed) {
    ...
    auto characterObjClone =
        std::make_shared<packets::PacketLobbyCharacterList>();
...
bool LobbyManager::HandlePacketLobbyWorldList(libcomp::ReadOnlyPacket &p) {
  ...
  bool changed = !mWorldList || obj->WorldsCount() != mWorldList->WorldsCount();
      ...
      if (!changed) {
        ...
        for (auto channel : world->GetChannels()) {
          ...
          changed |=
              ...
              originalChannel->GetVisibility() != channel->GetVisibility();
        }
      }
