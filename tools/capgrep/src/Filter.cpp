...
Filter::Filter(QWidget *p) : QDialog(p) {
  ...
  connect(ui.whiteList, SIGNAL(itemSelectionChanged()), this,
          SLOT(whiteSelectionChanged()));
  connect(ui.blackList, SIGNAL(itemSelectionChanged()), this,
          SLOT(blackSelectionChanged()));
...
void Filter::addWhite() {
  ...
  QString value = QInputDialog::getText(this, tr("White List Command"),
                                        tr("Command (hex):"), QLineEdit::Normal,
                                        QString(), &ok);
  ...
  if (!ok) {
    QMessageBox::critical(this, tr("Invalid Command Code"),
                          tr("The command code you entered is invalid. A hex "
                             "value between 0x0000 and 0xFFFF was expected."));
  ...
  if (mWhiteList.contains(cmd)) {
    QMessageBox::warning(
        this, tr("Duplicate Command Code"),
...
void Filter::removeWhite() {
  ...
  if (ui.whiteList->selectedItems().isEmpty()) return;
...
void Filter::removeBlack() {
  // Sanity check that an item is actually selected.
  if (ui.blackList->selectedItems().isEmpty()) return;
