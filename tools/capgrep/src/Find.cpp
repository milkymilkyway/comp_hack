...
Find::Find(PacketListFilter* model, QWidget* p) : QWidget(p) {
  ...
  QString searchType =
      settings.value("search_type", "binary").toString().toLower();
  ...
  connect(ui.cancelButton, SIGNAL(clicked(bool)), this, SLOT(cancelSearch()));
  connect(ui.findList, SIGNAL(doubleClicked(const QModelIndex&)), this,
          SLOT(doubleClicked(const QModelIndex&)));
  connect(ui.textButton, SIGNAL(toggled(bool)), this, SLOT(termTypeChanged()));
  connect(ui.binaryButton, SIGNAL(toggled(bool)), this,
          SLOT(termTypeChanged()));
  connect(ui.commandButton, SIGNAL(toggled(bool)), this,
          SLOT(termTypeChanged()));
...
void Find::findTerm() {
  ...
  if (ui.binaryButton->isChecked()) {
    ...
    if (term.length() % 2 != 0 || !QRegExp("[a-fA-F0-9]+").exactMatch(term)) {
      QMessageBox::critical(this, tr("Find Error"),
                            tr("A binary search term must consist solely of a "
                               "series of hex digit pairs."));
    ...
    while (!term.isEmpty()) {
      ...
      labelText.append(tr("%1")
                           .arg((uint32_t)byte & 0xFF, 2, 16, QLatin1Char('0'))
                           .toUpper());
    }
    ...
    if (!ok) {
      QMessageBox::critical(this, tr("Find Error"),
                            tr("A command code must have a hex value in the "
                               "range 0x0000-0xFFFF."));
    ...
    ui.searchLabel->setText(
        tr("Command: 0x%1")
            .arg(tr("%1").arg(cmd, 4, 16, QLatin1Char('0')).toUpper()));
...
void Find::findTerm(const QByteArray& term) {
  ...
  for (int i = 0; i < term.size(); i++) {
    ...
    labelText.append(
        tr("%1")
            .arg((uint32_t)term.at(i) & 0xFF, 2, 16, QLatin1Char('0'))
            .toUpper());
  }
...
void Find::doubleClicked(const QModelIndex& index) {
  ...
  MainWindow::getSingletonPtr()->showSelection(packet, offset,
                                               offset + term.size() - 1);
}
