    ...
    QWidget(pParent), mGameWorker(pWorker)
{
    ...
    connect(ui.characterList, SIGNAL(itemSelectionChanged()),
    ...
    const libcomp::Message::MessageClient *pMessage)
{
    ...
    ui.lastLogin->setText(tr("Last Login: %1").arg(lastLoginTime ?
        QDateTime::fromTime_t((uint)lastLoginTime).toString(
        tr("yyyy/MM/dd hh:mm")) : tr("----/--/-- --:--")));
...
bool LobbyScene::HandleConnectedToChannel(
        const libcomp::Message::MessageClient *pMessage)
{
    const logic::MessageConnectedToChannel *pMsg = reinterpret_cast<
        const logic::MessageConnectedToChannel*>(pMessage);
