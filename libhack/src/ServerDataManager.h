/**
 * @file libhack/src/ServerDataManager.h
 * @ingroup libhack
 *
 * @author HACKfrost
 *
...
#ifndef LIBHACK_SRC_SERVERDATAMANAGER_H
#define LIBHACK_SRC_SERVERDATAMANAGER_H
...
class ServerDataManager {
 public:
   ...
   */
  bool LoadData(libcomp::DataStore* pDataStore,
                DefinitionManager* definitionManager);
  ...
  std::list<std::shared_ptr<ServerScript>> LoadScripts(
      libcomp::DataStore* pDataStore, const libcomp::String& path,
      bool& success, bool store = true);
 ...
 private:
   ...
   */
  template <class T>
  bool LoadObjects(gsl::not_null<libcomp::DataStore*> pDataStore,
   ...
   */
  template <class T>
  bool LoadObjectsFromFile(gsl::not_null<libcomp::DataStore*> pDataStore,
    ...
    if (data.empty()) {
      LogServerDataManagerWarning([&]() {
        return libcomp::String("File does not exist or is empty: %1\n")
            .Arg(filePath);
      });
    ...
    while (nullptr != objNode) {
      if (!LoadObject<T>(objsDoc, objNode, definitionManager)) {
        LogServerDataManagerError([&]() {
          return libcomp::String("Failed to load XML file: %1\n").Arg(filePath);
        });
    ...
    LogServerDataManagerInfo([&]() {
      return libcomp::String("Loaded XML file: %1\n").Arg(filePath);
    });
   ...
   */
  bool LoadScripts(
      gsl::not_null<libcomp::DataStore*> pDataStore,
      const libcomp::String& datastorePath,
...
#endif  // LIBHACK_SRC_SERVERDATAMANAGER_H
