...
bool WorldServer::Initialize() {
  ...
  mDatabase = GetDatabase(configMap, true, GetDataStore(), "/migrations/world");
...
void WorldServer::FinishInitialize() {
  ...
  auto connectionManager =
      std::dynamic_pointer_cast<libcomp::Manager>(mManagerConnection);
  ...
  packetManager->AddParser<Parsers::AccountLogin>(
      to_underlying(InternalPacketCode_t::PACKET_ACCOUNT_LOGIN));
  packetManager->AddParser<Parsers::AccountLogout>(
      to_underlying(InternalPacketCode_t::PACKET_ACCOUNT_LOGOUT));
  packetManager->AddParser<Parsers::Relay>(
      to_underlying(InternalPacketCode_t::PACKET_RELAY));
  packetManager->AddParser<Parsers::DataSync>(
  ...
  packetManager->AddParser<Parsers::DataSync>(
      to_underlying(InternalPacketCode_t::PACKET_DATA_SYNC));
  packetManager->AddParser<Parsers::CharacterLogin>(
      to_underlying(InternalPacketCode_t::PACKET_CHARACTER_LOGIN));
  packetManager->AddParser<Parsers::FriendsUpdate>(
      to_underlying(InternalPacketCode_t::PACKET_FRIENDS_UPDATE));
  packetManager->AddParser<Parsers::PartyUpdate>(
      to_underlying(InternalPacketCode_t::PACKET_PARTY_UPDATE));
  packetManager->AddParser<Parsers::ClanUpdate>(
  ...
  std::thread serviceThread([&service]() { service.run(); });

  bool connected =
      libcomp::TcpConnection::STATUS_CONNECTED == lobbyConnection->GetStatus();
  ...
  if (nullptr == dynamic_cast<libcomp::Message::WorldNotification*>(pMessage)) {
    LogConnectionCriticalMsg(
        "Lobby server did not accept the world server notification.\n");
...
const std::shared_ptr<objects::RegisteredWorld>
WorldServer::GetRegisteredWorld() const {
...
WorldServer::GetChannelConnectionByID(int8_t channelID) const {
...
std::map<std::shared_ptr<libcomp::InternalConnection>,
         std::shared_ptr<objects::RegisteredChannel>>
WorldServer::GetChannels() const {
...
const std::shared_ptr<libcomp::InternalConnection>
WorldServer::GetLobbyConnection() const {
...
void WorldServer::RegisterChannel(
  ...
  const std::set<std::string> channelSyncTypes = {
      "CharacterLogin", "EventCounter",   "InstanceAccess", "Match",
      "MatchEntry",     "PentalphaEntry", "PentalphaMatch", "PvPMatch",
      "SearchEntry",    "StatusEffect",   "UBResult",       "UBTournament"};

  mSyncManager->RegisterConnection(connection, channelSyncTypes);
  mSyncManager->SyncExistingChannelRecords(connection);
}

bool WorldServer::RemoveChannel(
    const std::shared_ptr<libcomp::InternalConnection>& connection) {
  std::lock_guard<std::mutex> lock(mLock);
...
void WorldServer::SetLobbyDatabase(
...
bool WorldServer::RegisterServer() {
  ...
  auto channelServers =
      libcomp::PersistentObject::LoadAll<objects::RegisteredChannel>(mDatabase);
  ...
  if (nullptr == registeredWorld) {
    auto name = conf->GetName().IsEmpty()
                    ? libcomp::String("World %1").Arg(conf->GetID())
                    : conf->GetName();
    registeredWorld =
        std::shared_ptr<objects::RegisteredWorld>(new objects::RegisteredWorld);
    ...
    if (!registeredWorld->Register(registeredWorld) ||
        !registeredWorld->Insert(mLobbyDatabase)) {
      return false;
    }
  } else if (registeredWorld->GetStatus() ==
...
AccountManager* WorldServer::GetAccountManager() { return mAccountManager; }
...
uint32_t WorldServer::GetRelayPacket(libcomp::Packet& p,
                                     const std::list<int32_t>& targetCIDs,
                                     int32_t sourceCID) {
