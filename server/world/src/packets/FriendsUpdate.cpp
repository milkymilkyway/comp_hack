...
void FriendList(std::shared_ptr<WorldServer> server,
  ...
  auto fLogins =
      characterManager->GetRelatedCharacterLogins(cLogin, RELATED_FRIENDS);
  if (fLogins.size() == 0) {
...
void FriendRequest(std::shared_ptr<WorldServer> server,
  ...
  if (!failed) {
    ...
    if (!failed && channel) {
      ...
      request.WriteU8(
          (uint8_t)InternalPacketAction_t::PACKET_ACTION_YN_REQUEST);
...
void FriendRequestAccepted(
    std::shared_ptr<WorldServer> server,
    std::shared_ptr<libcomp::TcpConnection> sourceConnection,
    std::shared_ptr<objects::CharacterLogin> cLogin,
  ...
  if (!failed) {
    ...
    if (!failed) {
      ...
      request.WritePacketCode(InternalPacketCode_t::PACKET_FRIENDS_UPDATE);
      request.WriteU8(
          (uint8_t)InternalPacketAction_t::PACKET_ACTION_RESPONSE_YES);
      request.WriteS32Little(targetLogin->GetWorldCID());
      request.WriteString16Little(libcomp::Convert::Encoding_t::ENCODING_UTF8,
                                  sourceName, true);
...
void FriendRemoved(std::shared_ptr<WorldServer> server,
                   ...
                   int32_t targetCID) {
  auto targetLogin =
      server->GetCharacterManager()->GetCharacterLogin(targetCID);
  ...
  if (!failed) {
    ...
    auto worldDB = server->GetWorldDatabase();
    auto sourceFSettings =
        objects::FriendSettings::LoadFriendSettingsByCharacter(worldDB,
                                                               sourceUUID);
    auto targetFSettings =
        objects::FriendSettings::LoadFriendSettingsByCharacter(worldDB,
                                                               targetUUID);
...
bool Parsers::FriendsUpdate::Parse(
  ...
  if (!cLogin) {
    LogFriendError([&]() {
      return libcomp::String("Invalid world CID sent to FriendsUpdate: %1\n")
          .Arg(cid);
    });
  ...
  if ((InternalPacketAction_t)mode ==
    ...
    if (p.Left() < 4) {
      LogFriendError([&]() {
        return libcomp::String("Missing target CID parameter for command %1\n")
            .Arg(mode);
      });
    ...
    if (p.Left() < 2 || (p.Left() < (uint32_t)(2 + p.PeekU16Little()))) {
      LogFriendError([&]() {
        return libcomp::String("Missing source name parameter for command %1\n")
            .Arg(mode);
      });
    ...
    if (p.Left() < 2 || (p.Left() != (uint32_t)(2 + p.PeekU16Little()))) {
      LogGeneralError([&]() {
        return libcomp::String("Missing target name parameter for command %1\n")
            .Arg(mode);
      });
    ...
    switch ((InternalPacketAction_t)mode) {
      ...
      default:
        LogGeneralError([&]() {
          return libcomp::String("Unknown mode sent to FriendsUpdate: %1\n")
              .Arg(mode);
        });
