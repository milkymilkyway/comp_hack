...
bool Parsers::CharacterLogin::Parse(
  ...
  if (!cLogin) {
    LogGeneralError([&]() {
      return libcomp::String("Invalid world CID sent to CharacterLogin: %1\n")
          .Arg(cid);
    });
  ...
  if (!updateFlags) {
    ...
    if (cLogin->GetPartyID()) {
      ...
      if (member) {
        characterManager->SendPartyMember(member, cLogin->GetPartyID(), false,
                                          true, connection);
      }
    }
  ...
  if (member) {
    if (updateFlags &
      ...
      if (!member->LoadPacket(p, true)) {
        LogGeneralErrorMsg("CharacterLogin party member info failed to load\n");
    ...
    if (updateFlags &
      ...
      if (!partyDemon->LoadPacket(p, true)) {
        LogGeneralErrorMsg("CharacterLogin party demon info failed to load\n");
  ...
  bool partyMove =
      updateFlags & (uint8_t)CharacterLoginStateFlag_t::CHARLOGIN_ZONE &&
      cLogin->GetPartyID();

  // Send all party flags if in a party and changing zones to members in that
  // zone
  if (partyMove) {
    updateFlags =
        updateFlags | (uint8_t)CharacterLoginStateFlag_t::CHARLOGIN_PARTY_FLAGS;
  }
  ...
  // If changing zones and in a party, update party member visibility (inverse
  // of previous)
  if (partyMove) {
    auto partyMembers =
        characterManager->GetRelatedCharacterLogins(cLogin, RELATED_PARTY);

    // Send all up to date info on party members in the zone they just entered
    // or the left
    bool queued = false;
    for (auto login : partyMembers) {
      ...
      if ((login->GetZoneID() != cLogin->GetZoneID() ||
           login->GetChannelID() != cLogin->GetChannelID()) &&
          !previousZone) {
        continue;
      }
      ...
      if (characterManager->GetStatusPacket(reply, login, outFlags) &&
          (previousZone ||
           outFlags != (uint8_t)CharacterLoginStateFlag_t::CHARLOGIN_ZONE)) {
