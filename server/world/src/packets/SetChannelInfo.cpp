...
bool Parsers::SetChannelInfo::Parse(
  ...
  if (p.Size() == 0) {
    LogGeneralDebugMsg(
        "Channel Server connection sent an empty response. The connection will "
        "be closed.\n");
  ...
  if (server->GetChannelConnectionByID((int8_t)channelID)) {
    LogGeneralDebugMsg(
        "The ID of the channel requesting a connection does not match an "
  ...
  auto svr =
      objects::RegisteredChannel::LoadRegisteredChannelByID(worldDB, channelID);

  connection->SetName(libcomp::String("%1:%2:%3")
  ...
  packet.WriteU8(to_underlying(InternalPacketAction_t::PACKET_ACTION_UPDATE));
