...
bool Parsers::AccountLogout::Parse(
  ...
  if (action == LogoutPacketAction_t::LOGOUT_CHANNEL_SWITCH) {
    if (!isLoggedIn || !cLogin) {
      LogGeneralDebug([&]() {
        return libcomp::String(
                   "Channel switch requested for user not currently logged in: "
                   "'%1'\n")
            .Arg(username);
      });
    ...
    if (!channelLogin->LoadPacket(p)) {
      LogGeneralErrorMsg("Failed to load channel switch info from channel\n");
    ...
    if (accountManager->SwitchChannel(login, channelLogin)) {
      ...
      reply.WritePacketCode(InternalPacketCode_t::PACKET_ACCOUNT_LOGOUT);
    ...
    switch (disconnectType) {
      case -1: {
        ...
        if (!relogin->LoadPacket(p)) {
          LogGeneralErrorMsg(
              "Failed to load channel disconnect info from channel\n");
        ...
        server->GetWorldSyncManager()->PushRelogin(relogin, instanceID);
      } break;
      case 1:
        if (!cLogin) {
          LogGeneralDebug([&]() {
            return libcomp::String(
                       "Special channel disconnect failed because user is not "
      ...
      case 3: {
        ...
        if (disconnectType == 2) {
          if (cLogin && server->GetCharacterManager()->RequestChannelDisconnect(
                            cLogin->GetWorldCID())) {
            LogGeneralDebug([&]() {
              return libcomp::String(
                         "Requesting special channel disconnect: '%1'\n")
                  .Arg(username);
            });
            ...
            LogGeneralDebug([&]() {
              return libcomp::String(
                         "Special channel disconnect failed to find channel: "
                         "'%1'\n")
                  .Arg(username);
            });
          }
        ...
        if (server->GetAccountManager()->LogoutUser(username, -1)) {
          ...
          LogGeneralDebug([&]() {
            return libcomp::String(
                       "Special channel disconnect user not on this world: "
                       "'%1'\n")
                .Arg(username);
          });
        }
        ...
        request.WritePacketCode(InternalPacketCode_t::PACKET_ACCOUNT_LOGOUT);
      ...
      default:
        LogGeneralError([&]() {
          return libcomp::String("Unknown logout request received %1: '%2'\n")
              .Arg(disconnectType)
              .Arg(username);
        });
        break;
    ...
    if (!isLoggedIn || !login) {
      LogGeneralDebug([&]() {
        return libcomp::String(
                   "Logout requested for user not currently logged in: '%1'\n")
            .Arg(username);
      });
    ...
    if (accountManager->ChannelSwitchPending(username, channelID)) {
      LogGeneralDebug([&]() {
        return libcomp::String("User is switching to channel %1: '%2'\n")
            .Arg(channelID)
            .Arg(username);
      });
