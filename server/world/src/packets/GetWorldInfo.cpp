...
bool Parsers::GetWorldInfo::Parse(
  ...
  if (fromLobby) {
    ...
    if (!dbConfig->LoadPacket(p, false)) {
      LogGeneralCriticalMsg(
          "The lobby did not supply a valid database connection configuration "
    ...
    if (!server->RegisterServer()) {
      LogGeneralCriticalMsg(
          "The server failed to register with the lobby's database. Notifying "
          "the lobby of the failure.\n");
    }
    ...
    if (!syncManager->Initialize() ||
        !syncManager->RegisterConnection(server->GetLobbyConnection(),
                                         lobbyTypes)) {
    ...
    server->GetTimerManager()->SchedulePeriodicEvent(
        sch,
        [](WorldServer* pServer) {
        ...
        if (server->GetChannelConnectionByID(reservedID)) {
          LogGeneralError([&]() {
            return libcomp::String(
                       "Channel requested reserved ID %1 which has already "
                       "been given to another server\n")
                .Arg(reservedID);
          });
