...
class CharacterManager {
 public:
   ...
   *  of CIDs should be inserted. If the value is larger than the packet, it
   * will be appended to the end.
   * @return false if an error occurs
   */
  bool SendToCharacters(
      libcomp::Packet& p,
   ...
   */
  void ConvertToTargetCIDPacket(libcomp::Packet& p, uint32_t cidOffset,
                                size_t cidCount);
   ...
   *  of CIDs should be inserted. If the value is larger than the packet, it
   * will be appended to the end.
   * @param relatedTypes Flags indicating what types of related characters to
   * send the packet to. Use the constants in the CharacterManager header to
   * signify these.
  ...
  bool SendToRelatedCharacters(libcomp::Packet& p, int32_t worldCID,
                               uint32_t cidOffset, uint8_t relatedTypes,
   ...
   */
  std::list<std::shared_ptr<objects::CharacterLogin>> GetRelatedCharacterLogins(
      std::shared_ptr<objects::CharacterLogin> cLogin, uint8_t relatedTypes);

  /**
   * Send packets containing CharacterLogin information about the supplied
   * logins contextual to other related characters
  ...
  bool GetStatusPacket(libcomp::Packet& p,
                       const std::shared_ptr<objects::CharacterLogin>& cLogin,
                       uint8_t& updateFlags);
   ...
   */
  void PartyDisband(
      uint32_t partyID, int32_t sourceCID = 0,
   ...
   */
  void PartyKick(std::shared_ptr<objects::CharacterLogin> cLogin,
                 int32_t targetCID);
   ...
   */
  bool ClanJoin(std::shared_ptr<objects::CharacterLogin> cLogin,
                int32_t clanID);
   ...
   *  were not
   */
  bool ClanLeave(std::shared_ptr<objects::CharacterLogin> cLogin,
                 int32_t clanID,
                 std::shared_ptr<libcomp::TcpConnection> requestConnection);
   ...
   */
  void ClanDisband(
      int32_t clanID, int32_t sourceCID = 0,
      std::shared_ptr<libcomp::TcpConnection> requestConnection = nullptr);
 ...
 private:
  ...
  std::unordered_map<libcomp::String, std::shared_ptr<objects::CharacterLogin>>
      mCharacterMap;
  ...
  std::unordered_map<int32_t, std::shared_ptr<objects::PartyCharacter>>
      mPartyCharacters;
