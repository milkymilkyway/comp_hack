...
bool AccountManager::ChannelLogin(
  ...
  if (!character || !account) {
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "CharacterLogin encountered with no account or character "
                 "loaded: %1.\n")
  ...
  const static int32_t timeAdjust =
  ...
  if (timeAdjust) {
    ...
    uint32_t adjustToday = (uint32_t)((now + timeAdjust) / DAY_SEC * DAY_SEC);
  ...
  if (lastLogin && today > lastLogin) {
    ...
    LogAccountManagerDebug([character, daysSinceLogin]() {
      return libcomp::String(
                 "Performing one time daily character login actions for "
                 "character %1 (last login %2 day(s) ago).\n")
    ...
    if (!character->GetCOMP().IsNull() && progress &&
      ...
      if (character->LoadCOMP(worldDB)) {
        auto demons = objects::Demon::LoadDemonListByDemonBox(
            worldDB, character->GetCOMP().GetUUID());
        for (auto demon : demons) {
          // Set the quest if familiarity is high enough
          if (!demon->GetHasQuest() && demon->GetFamiliarity() >= 4001) {
        ...
        LogAccountManagerError([&]() {
          return libcomp::String(
                     "Failed to load COMP to update demon quests on account: "
                     "%1.\n")
    ...
    if (stats->GetLevel() > 0) {
      ...
      points =
          points + (int32_t)ceil((float)stats->GetLevel() * 0.2 *
  ...
  if (!worldDB->ProcessChangeSet(worldChanges) || !account->Update(lobbyDB)) {
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "Failed to update character data during channel login request "
                 "for account: %1.\n")
          .Arg(account->GetUsername());
    });
...
bool AccountManager::SwitchChannel(
  ...
  auto username = login->GetAccount()->GetUsername();
  {
    ...
    if (login->GetState() != objects::AccountLogin::State_t::CHANNEL) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "Channel switch for account '%1' failed because it is not "
    ...
    LogAccountManagerDebug([username, switchDef]() {
      return libcomp::String(
                 "Channel switch requested from channel %1 to %2 by user: "
                 "'%3'\n")
          .Arg(switchDef->GetFromChannel())
          .Arg(switchDef->GetToChannel())
          .Arg(username);
    });
  ...
  server->GetTimerManager()->ScheduleEventIn(
      static_cast<int>(config->GetChannelConnectionTimeOut()),
      [server](const std::shared_ptr<WorldServer> pServer,
...
std::shared_ptr<objects::AccountLogin> AccountManager::LogoutUser(
  ...
  if (result) {
    ...
    if (cLogin && !cLogin->GetCharacter().IsNull()) {
      ...
      lobbyMessage.WritePacketCode(InternalPacketCode_t::PACKET_ACCOUNT_LOGOUT);
...
bool AccountManager::ExpireSession(const libcomp::String& username,
  ...
  libcomp::String lookup = username.ToLower();
  {
    ...
    if (mAccountMap.end() != pair) {
      ...
      if (login &&
          objects::AccountLogin::State_t::CHANNEL != login->GetState() &&
          key == login->GetSessionKey()) {
        LogAccountManagerDebug([&]() {
          return libcomp::String("Session for username '%1' has expired.\n")
              .Arg(username);
        });
  ...
  if (expire) {
    ...
    auto cLogin = login->GetCharacterLogin();
    if (cLogin) {
      mServer.lock()->GetCharacterManager()->RequestChannelDisconnect(
          cLogin->GetWorldCID());
    }
...
void AccountManager::HandleLobbyLogin(
  ...
  LogAccountManagerDebug([login]() {
    return libcomp::String("Lobby login request received from user: '%1'\n")
        .Arg(login->GetAccount().GetUUID().ToString());
  });
  ...
  auto cLogin = server->GetCharacterManager()->GetCharacterLogin(
      login->GetCharacterLogin()->GetCharacter().GetUUID());
  auto channelLogin =
      cLogin ? server->GetWorldSyncManager()->PopRelogin(cLogin->GetWorldCID())
             : nullptr;
  ...
  if (!channelLogin &&
    ...
    if (primaryChannel) {
      ...
      LogAccountManagerDebug([login]() {
        return libcomp::String("Requesting login channel for user: '%1'\n")
            .Arg(login->GetAccount().GetUUID().ToString());
      });
    } else {
      // No primary channel, return failure
      auto account = login->GetAccount().Get(server->GetLobbyDatabase());
      ...
      LogAccountManagerErrorMsg(
          "Lobby login failed because there is no primary login channel "
          "configured.");
    }
  } else {
...
void AccountManager::CompleteLobbyLogin(
  ...
  if (!account) {
    LogAccountManagerError([&]() {
      return libcomp::String("Invalid account sent to world AccountLogin: %1\n")
          .Arg(login->GetAccount().GetUUID().ToString());
    });
    ...
    if (cUUID.IsNull() || !cLogin->GetCharacter().Get(worldDB, true)) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "Character UUID '%1' is not valid for this world.\n")
            .Arg(cUUID.ToString());
      });
  ...
  ok &=
      channelID >= 0 && server->GetChannelConnectionByID(channelID) != nullptr;
  if (ok) {
    ...
    if (!LobbyLogin(login)) {
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "Failed to login character '%1'. Here is the state of the "
                   "login object now: %2\n")
            .Arg(account->GetUsername())
            .Arg(login->GetXml());
      });
  ...
  if (ok) {
    ...
    if (!character->GetClan().IsNull()) {
      ...
      if (!clan) {
        clan = libcomp::PersistentObject::LoadObjectByUUID<objects::Clan>(
            worldDB, character->GetClan().GetUUID());
      }
...
void AccountManager::HandleChannelLogin(
  ...
  if (nullptr == channel) {
    LogAccountManagerErrorMsg(
        "AccountLogin request received from a connection not belonging to the "
        "lobby or any connected channel.\n");
    ...
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "Account with username '%1' is not logged in to this world or "
    ...
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "Account with username '%1' is not logged in to this world "
                 "(bad cLogin).\n")
          .Arg(username);
    });
    ...
    LogAccountManagerErrorMsg(
        "AccountLogin request received from a channel not matching the "
        "account's current login information.\n");

    ok = false;
  } else if (login->GetSessionKey() != sessionKey) {
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "Invalid session key provided for account with username '%1': "
                 "Expected %2, found %3\n")
          .Arg(username)
          .Arg(login->GetSessionKey())
          .Arg(sessionKey);
  ...
  if (ok) {
    if (ChannelLogin(login)) {
      ...
      libcomp::Packet lobbyMessage;
      lobbyMessage.WritePacketCode(InternalPacketCode_t::PACKET_ACCOUNT_LOGIN);
  ...
  if (!ok) {
    // Faiure, send the username back to disconnect
    reply.WriteS8(0);
    reply.WriteString16Little(libcomp::Convert::ENCODING_UTF8, username, true);
  }
...
void AccountManager::UpdateSessionKey(
    std::shared_ptr<objects::AccountLogin> login) {
  login->SetSessionKey(RNG(uint32_t, 1, (uint32_t)0x7FFFFFFF));
}

void AccountManager::PushChannelSwitch(
    const libcomp::String& username,
    const std::shared_ptr<objects::ChannelLogin>& switchDef) {
  libcomp::String lookup = username.ToLower();
...
void AccountManager::CleanupAccountWorldData() {
  ...
  auto accountWorldDataList =
      objects::AccountWorldData::LoadAccountWorldDataListByCleanupRequired(
          worldDB, true);
  ...
  for (auto accountWorldData : accountWorldDataList) {
    auto account =
        libcomp::PersistentObject::LoadObjectByUUID<objects::Account>(
            lobbyDB, accountWorldData->GetAccount().GetUUID());

    if (account) {
      LogAccountManagerDebug([&]() {
        return libcomp::String(
                   "Cleaning up AccountWorldData associated to account: %1\n")
...
bool AccountManager::DeleteCharacter(
  ...
  auto cLogin = characterManager->GetCharacterLogin(character->GetName());

  int32_t clanID = 0;
  auto clanMember =
      objects::ClanMember::LoadClanMemberByCharacter(db, characterUUID);
  if (clanMember) {
  ...
  auto friendSettings =
      objects::FriendSettings::LoadFriendSettingsByCharacter(db, characterUUID);
  if (friendSettings && friendSettings->FriendsCount() > 0) {
    ...
    for (auto otherChar : friendSettings->GetFriends()) {
      auto otherFriendSettings =
          objects::FriendSettings::LoadFriendSettingsByCharacter(db, otherChar);
      if (otherFriendSettings) {
        auto friends = otherFriendSettings->GetFriends();
        friends.remove_if(
            [characterUUID](
                const libcomp::ObjectReference<libcomp::PersistentObject>& f) {
  ...
  if (cLogin && cLogin->GetChannelID() >= 0) {
    LogAccountManagerWarning([&]() {
      return libcomp::String(
                 "Deleting character '%1' is still logged in on account: %2\n")
          .Arg(character->GetName())
          .Arg(character->GetAccount().ToString());
    });
    ...
    auto accountWorldData =
        objects::AccountWorldData::LoadAccountWorldDataByAccount(
            db, character->GetAccount());
    if (accountWorldData) {
      ...
      LogAccountManagerError([&]() {
        return libcomp::String(
                   "Failed to delete logged in character without associated "
                   "AccountWorlData: %1\n")
            .Arg(characterUUID.ToString());
      });
  ...
  for (auto itemBox :
       objects::ItemBox::LoadItemBoxListByCharacter(db, characterUUID)) {
    for (auto item :
  ...
  for (auto demonBox :
       objects::DemonBox::LoadDemonBoxListByCharacter(db, characterUUID)) {
    for (auto demon :
         objects::Demon::LoadDemonListByDemonBox(db, demonBox->GetUUID())) {
      entityUIDs.push_back(demon->GetUUID());

      for (auto iSkill : objects::InheritedSkill::LoadInheritedSkillListByDemon(
               db, demon->GetUUID())) {
  ...
  // Delete expertise
  for (auto expertise :
       objects::Expertise::LoadExpertiseListByCharacter(db, characterUUID)) {
  ...
  // Delete hotbar
  for (auto hotbar :
       objects::Hotbar::LoadHotbarListByCharacter(db, characterUUID)) {
  ...
  // Delete quests
  for (auto quest :
       objects::Quest::LoadQuestListByCharacter(db, characterUUID)) {
  ...
  for (libobjgen::UUID entityUID : entityUIDs) {
    auto entityStats =
        objects::EntityStats::LoadEntityStatsByEntity(db, entityUID);
    if (entityStats) {
    ...
    for (auto status :
         objects::StatusEffect::LoadStatusEffectListByEntity(db, entityUID)) {
  ...
  auto progress = objects::CharacterProgress::LoadCharacterProgressByCharacter(
      db, characterUUID);
  if (progress) {
  ...
  LogAccountManagerWarning([&]() {
    return libcomp::String("Failed to delete character '%1' on account: %2\n")
        .Arg(character->GetName())
        .Arg(character->GetAccount().ToString());
  });
...
bool AccountManager::StartWebGameSession(
  ...
  if (lobby) {
    ...
    notify.WriteString16Little(libcomp::Convert::Encoding_t::ENCODING_UTF8,
                               account->GetUsername(), true);
...
void AccountManager::Cleanup(
  ...
  Cleanup<objects::FriendSettings>(
      cLogin->GetCharacter()->GetFriendSettings().Get());
  Cleanup<objects::PvPData>(cLogin->GetCharacter()->GetPvPData().Get());
