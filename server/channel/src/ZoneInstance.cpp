...
ScriptEngine& ScriptEngine::Using<ZoneInstance>() {
  if (!BindingExists("ZoneInstance", true)) {
    ...
    Sqrat::DerivedClass<ZoneInstance, objects::ZoneInstanceObject,
                        Sqrat::NoConstructor<ZoneInstance>>
        binding(mVM, "ZoneInstance");
    binding.Func("GetDefinitionID", &ZoneInstance::GetDefinitionID)
...
ZoneInstance::ZoneInstance(
    uint32_t id, const std::shared_ptr<objects::ServerZoneInstance>& definition,
...
uint32_t ZoneInstance::GetDefinitionID() { return GetDefinition()->GetID(); }
...
bool ZoneInstance::AddZone(const std::shared_ptr<Zone>& zone) {
  ...
  if (mZones.find(def->GetID()) == mZones.end() ||
      mZones[def->GetID()].find(def->GetDynamicMapID()) ==
          mZones[def->GetID()].end()) {
