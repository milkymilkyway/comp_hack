...
class MatchManager {
 public:
   ...
   */
  void IncreaseBaseBonus(int32_t baseID, uint32_t zoneID, uint32_t instanceID,
   ...
   */
  int32_t UpdatePvPPoints(uint32_t instanceID,
  ...
  int32_t UpdatePvPPoints(uint32_t instanceID, int32_t sourceID,
                          int32_t targetID, uint8_t teamID, int32_t points,
                          bool altMode);
   ...
   */
  bool ToggleDiasporaBase(const std::shared_ptr<Zone>& zone, int32_t baseID,
                          int32_t sourceEntityID, bool capture);
   ...
   * @return true if the base was updated, false if it was not
   */
  bool ResetDiasporaBase(uint32_t zoneID, uint32_t instanceID, int32_t baseID,
                         uint64_t resetTime);
  ...
  bool StartUltimateBattleTimer(const std::shared_ptr<Zone>& zone,
                                uint32_t duration,
                                const libcomp::String& eventID, bool endPhase);
 ...
 private:
   ...
   */
  void SendPhase(
      const std::shared_ptr<Zone>& zone, bool timerStart = false,
      const std::shared_ptr<ChannelClientConnection>& client = nullptr);
   ...
   *  the entire zone
   */
  void SendUltimateBattleMembers(
      const std::shared_ptr<Zone>& zone,
      const std::shared_ptr<ChannelClientConnection>& client = nullptr);
   ...
   *  the entire zone
   */
  void SendUltimateBattleMemberState(
      const std::shared_ptr<Zone>& zone,
      const std::shared_ptr<ChannelClientConnection>& client = nullptr);
  ...
  std::unordered_map<int32_t, std::shared_ptr<objects::MatchEntry>>
      mMatchEntries;
