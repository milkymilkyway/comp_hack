...
class ManagerConnection : public libcomp::Manager {
 public:
   ...
   */
  const std::shared_ptr<ChannelClientConnection> GetEntityClient(
      int32_t id, bool worldID = false);
   ...
   */
  std::list<std::shared_ptr<ChannelClientConnection>> GetEntityClients(
      std::set<int32_t> ids, bool worldID = false);
   ...
   *  packet
   */
  std::list<std::shared_ptr<ChannelClientConnection>> GatherWorldTargetClients(
      libcomp::ReadOnlyPacket& p, bool& success);
   ...
   */
  std::list<std::shared_ptr<ChannelClientConnection>> GetPartyConnections(
 ...
 private:
  ...
  std::unordered_map<libcomp::String, std::shared_ptr<ChannelClientConnection>>
      mClientConnections;
