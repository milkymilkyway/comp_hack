...
class EventManager {
 public:
  ...
  bool RequestMenu(const std::shared_ptr<ChannelClientConnection>& client,
                   int32_t menuType, int32_t shopID = 0,
                   int32_t sourceEntityID = 0, bool allowInsert = false);
  ...
  bool UpdateQuest(
      ...
      int8_t phase, bool forceUpdate = false,
   ...
   */
  int8_t EndDemonQuest(const std::shared_ptr<ChannelClientConnection>& client,
                       int8_t failCode = 2);
 ...
 private:
   ...
   *  the current phase
   */
  bool EvaluateQuestPhaseRequirements(
      const std::shared_ptr<ChannelClientConnection>& client, int16_t questID,
      int8_t phase);
  ...
  bool Compare(
      ...
      EventCompareMode defaultCompare = EventCompareMode::DEFAULT_COMPARE,
      uint16_t validCompareSetting = 63);
  ...
  std::shared_ptr<T> GetEvent(EventContext& ctx) {
    ...
    if (ptr && !ptr->GetTransformScriptID().IsEmpty()) {
      ...
      if (PrepareTransformScript(ctx, engine)) {
        ...
        Sqrat::Function f(Sqrat::RootTable(engine->GetVM()), "prepare");
        auto scriptResult = !f.IsNull() ? f.Evaluate<int32_t>(ptr) : 0;

        // Apply the transformation
        if (scriptResult && *scriptResult == 0 && TransformEvent(ctx, engine) &&
            VerifyITime(ctx, ptr)) {
          // Set new event
          inst->SetEvent(ptr);
          return ptr;
