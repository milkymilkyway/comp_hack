...
class CharacterManager {
 public:
   ...
   * @param client Pointer to the client connection
   */
  void SendPartnerData(const std::shared_ptr<ChannelClientConnection>& client);
   ...
   */
  void SendDemonData(const std::shared_ptr<ChannelClientConnection>& client,
   ...
   */
  void SendEntityStats(std::shared_ptr<ChannelClientConnection> client,
                       int32_t entityID, bool includeSelf = true);
   ...
   *  in constants.
   */
  void ReviveCharacter(std::shared_ptr<ChannelClientConnection> client,
                       int32_t revivalMode);
   ...
   */
  static bool GetEntityRevivalPacket(
   ...
   * @param client Pointer to the client connection
   */
  void SendAutoRecovery(const std::shared_ptr<ChannelClientConnection>& client);
  ...
  void SummonDemon(
      const std::shared_ptr<channel::ChannelClientConnection>& client,
      int64_t demonID, bool updatePartyState = true);
   ...
   *  been updated
   */
  void SendDemonBoxData(const std::shared_ptr<ChannelClientConnection>& client,
                        int8_t boxID, std::set<int8_t> slots = {});
   ...
   */
  void SendItemBoxData(const std::shared_ptr<ChannelClientConnection>& client,
   ...
   *  should be sent right away
   */
  void SendLootItemData(
      const std::list<std::shared_ptr<ChannelClientConnection>>& clients,
      const std::shared_ptr<LootBoxState>& lState, bool queue = false);
  ...
  bool ReunionDemon(const std::shared_ptr<ChannelClientConnection> client,
                    ...
                    bool replyToClient, bool force = false,
                    int8_t forceRank = -1);
  ...
  void UpdateFamiliarity(
      const std::shared_ptr<channel::ChannelClientConnection>& client,
      int32_t familiarity, bool isAdjust = false, bool sendPacket = true);
   ...
   */
  int32_t UpdateSoulPoints(
      const std::shared_ptr<channel::ChannelClientConnection>& client,
      int32_t points, bool isAdjust = false, bool applyRate = false);
   ...
   *  value, false if it should replace the curent value
   */
  bool UpdateBP(const std::shared_ptr<channel::ChannelClientConnection>& client,
                int32_t points, bool isAdjust);
   ...
   * @return true if the update succeeded, false if it did not
   */
  bool UpdateEventCounter(
      const std::shared_ptr<channel::ChannelClientConnection>& client,
      int32_t type, int32_t value, bool noSync = false);
   ...
   */
  void LevelUp(const std::shared_ptr<channel::ChannelClientConnection>& client,
   ...
   */
  bool AddMap(const std::shared_ptr<channel::ChannelClientConnection>& client,
              uint16_t mapID);
   ...
   * @param client Pointer to the client connection
   */
  void SendMapFlags(const std::shared_ptr<ChannelClientConnection>& client);
   ...
   * @param client Pointer to the client connection
   */
  void SendPluginFlags(const std::shared_ptr<ChannelClientConnection>& client);
   ...
   */
  void SendMaterials(const std::shared_ptr<ChannelClientConnection>& client,
   ...
   * @param client Pointer to the client connection
   */
  void SendDevilBook(const std::shared_ptr<ChannelClientConnection>& client);
   ...
   *  should be sent right away
   */
  void SendInvokeStatus(const std::shared_ptr<ChannelClientConnection>& client,
                        bool force, bool queue = false);
   ...
   */
  bool UpdateStatusEffects(const std::shared_ptr<ActiveEntityState>& eState,
   ...
   */
  bool AddRemoveOpponent(bool add,
                         const std::shared_ptr<ActiveEntityState>& eState1,
   ...
   */
  void CalculateCharacterBaseStats(
   ...
   */
  static void FamiliarityBoostStats(
   ...
   *  Defaults to all
   */
  static void CalculateDependentStats(
   ...
   */
  uint32_t GetDemonPresent(uint32_t demonType, int8_t level,
   ...
   */
  void GetItemDetailPacketData(libcomp::Packet& p,
                               const std::shared_ptr<objects::Item>& item,
 ...
 private:
  ...
  static void BoostStats(libcomp::EnumMap<CorrectTbl, int32_t>& stats,
                         const std::shared_ptr<objects::MiDevilLVUpData>& data,
                         int boostLevel);
