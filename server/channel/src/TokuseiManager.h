...
typedef objects::TokuseiSkillCondition ::SkillConditionType_t
    TokuseiSkillConditionType;
...
class TokuseiManager {
 public:
  ...
  /**
   * Recalculate the tokusei effects on the supplied entity and any related
   * entities if any of the specified changes are triggers on the entity.
   * @param eState Pointer to the entity that has changed
   * @param changes Changes that could trigger a tokusei recalculation
   * @return Map of entity IDs to a true value if they have had their stats
   * recalculated or false if only their tokusei sets and triggers were updated
   */
  std::unordered_map<int32_t, bool> Recalculate(
   ...
   * @param recalcStats false if the effect tokusei should be determined but the
   * entities should not have their stats recalculated, true if both should
   * occur
   * @param ignoreStateRecalc Set of entity IDs to ignore when recalculating
   * stats
   * @return Map of entity IDs to a true value if they have had their stats
   * recalculated or false if only their tokusei sets and triggers were updated
   */
   ...
   * recalculated or false if only their tokusei sets and triggers were updated
   */
  std::unordered_map<int32_t, bool> RecalculateParty(
      const std::shared_ptr<objects::Party>& party);

  /**
   * Get all entities that could be affected by any other tokusei effect from
   * another entity in the list, starting with the supplied entity.
   ...
   */
  bool EvaluateTokuseiConditions(
      const std::shared_ptr<ActiveEntityState>& eState,
  ...
  /**
   * Calculate the sum of all instances of a specific aspect value on the
   * supplied entity.
   * @param eState Pointer to the tokusei source
  ...
  /**
   * Calculate the sum of all instances of a specific aspect's modifier values
   * keyed on shared aspect value on the supplied entity.
   * @param eState Pointer to the tokusei source
   ...
   */
  std::unordered_map<int32_t, double> GetAspectMap(
      const std::shared_ptr<ActiveEntityState>& eState, TokuseiAspectType type,
      std::shared_ptr<objects::CalculatedEntityState> calcState = nullptr);
   ...
   * @param calcState Override CalculatedEntityState to use instead of the
   * entity's default
   * @return Map of calculated aspect modifier value sums by aspect value with a
   ...
   */
  std::list<double> GetAspectValueList(
      const std::shared_ptr<ActiveEntityState>& eState, TokuseiAspectType type,
      std::shared_ptr<objects::CalculatedEntityState> calcState = nullptr);
 ...
 private:
   ...
   * @param numericCompare If false and a numeric comparator type is on the
   * condition this will always return false
   * @return false if the condition does not evaluate to true
   */
  bool Compare(int32_t value,
   ...
   * @return false if the condition does not evaluate to true
   */
  bool Compare(int32_t value1, int32_t value2,
               std::shared_ptr<objects::TokuseiCondition> condition,
               bool numericCompare) const;
  ...
  /// Quick access mapping of constant status effect IDs to their source tokusei
  /// IDs
