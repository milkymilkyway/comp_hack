...
ManagerConnection::GetWorldConnection() const {
  return mWorldConnection;
}

void ManagerConnection::SetWorldConnection(
    const std::shared_ptr<libcomp::InternalConnection>& worldConnection) {
  mWorldConnection = worldConnection;
}

const std::shared_ptr<ChannelClientConnection>
ManagerConnection::GetClientConnection(const libcomp::String& username) {
...
void ManagerConnection::SetClientConnection(
    const std::shared_ptr<ChannelClientConnection>& connection) {
...
std::list<std::shared_ptr<ChannelClientConnection>>
ManagerConnection::GatherWorldTargetClients(libcomp::ReadOnlyPacket& p,
                                            bool& success) {
  std::list<std::shared_ptr<ChannelClientConnection>> results;
  success = false;
...
std::list<std::shared_ptr<ChannelClientConnection>>
ManagerConnection::GetPartyConnections(
...
bool ManagerConnection::ScheduleClientTimeoutHandler(uint16_t timeout) {
  ...
  return server->ScheduleWork(
      nextTime,
      [](ChannelServer* svr, uint16_t t) {
