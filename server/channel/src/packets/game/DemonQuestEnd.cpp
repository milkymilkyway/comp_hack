...
bool Parsers::DemonQuestEnd::Parse(
  ...
  int8_t failCode =
      demon ? server->GetEventManager()->EndDemonQuest(client, 0) : 2;
  if (!failCode) {
    ...
    for (auto& pair : dQuest->GetRewardItems()) {
      ...
      uint32_t newStack = it != addItems.end() ? it->second : 0;
    ...
    if (xp) {
      characterManager->UpdateExperience(client, xp, cState->GetEntityID());
    }
  } else if (failCode != -1) {
    // Fail the quest
