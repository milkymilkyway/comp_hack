...
bool Parsers::StopMovement::Parse(
  ...
  if (nullptr == eState) {
    LogGeneralError([&]() {
      return libcomp::String(
                 "Invalid entity ID received from a stop movement request: "
                 "%1\n")
  ...
  if (moveCorrection) {
    ...
    if (zoneManager->CorrectClientPosition(eState, dest)) {
      LogGeneralDebug([&]() {
        return libcomp::String(
                   "Player movement stop corrected in zone %1: %2\n")
  ...
  if (positionCorrected || eState->IsClientVisible()) {
    auto zConnections =
        zoneManager->GetZoneConnections(client, positionCorrected);
