 ...
 * @author HACKfrost
 *
 * @brief Request from the client to rent a client account item/demon
 * depository.
 *
...
bool Parsers::DepoRent::Parse(
  ...
  if (nullptr == item) {
    LogGeneralErrorMsg("Depo rental failed due to unknown purchase item ID.\n");
  } else {
    ...
    if (isItemDepo) {
      ...
      LogGeneralErrorMsg(
          "Depo rental failed due to unknown/invalid purchase item type.\n");
    }

    if (success) {
      // Set the expiration timestamp to the specified offset past the current
      // time
      ...
      if (isItemDepo) {
        if (itemDepo->GetRentalExpiration() > timestamp) {
          // Extend current time
          delta =
              delta + (int32_t)(itemDepo->GetRentalExpiration() - timestamp);
        }

        itemDepo->SetRentalExpiration((uint32_t)(timestamp + (uint32_t)delta));
        ...
        if (demonDepo->GetRentalExpiration() > timestamp) {
          // Extend current time
          delta =
              delta + (int32_t)(demonDepo->GetRentalExpiration() - timestamp);
        }

        demonDepo->SetRentalExpiration((uint32_t)(timestamp + (uint32_t)delta));
