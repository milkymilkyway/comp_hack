  ...
  if (!itemUUID.IsNull()) {
    auto postItem =
        libcomp::PersistentObject::LoadObjectByUUID<objects::PostItem>(
            lobbyDB, itemUUID);
    auto productData =
        postItem ? definitionManager->GetShopProductData(postItem->GetType())
                 : nullptr;
    ...
    if (productData) {
      ...
      // the post is more damaging to the player than a server caused
      // duplication so only delete the post item after the item has been
      // created.
