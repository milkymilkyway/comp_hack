...
bool Parsers::TeamInvite::Parse(
  ...
  if (errorCode == (int8_t)TeamErrorCodes_t::SUCCESS) {
    ...
    if (it != SVR_CONST.TEAM_VALUABLES.end()) {
      for (uint16_t valuableID : it->second) {
        hasValuables &= characterManager->HasValuable(targetCState->GetEntity(),
                                                      valuableID);
      }
    }
  ...
  if (errorCode == (int8_t)TeamErrorCodes_t::SUCCESS &&
    ...
    if (relayError) {
      reply.Clear();
      reply.WritePacketCode(ChannelToClientPacketCode_t::PACKET_TEAM_ANSWERED);
      reply.WriteS32Little(teamID);
      reply.WriteS8(targetError);
      reply.WriteString16Little(state->GetClientStringEncoding(), targetName,
                                true);
