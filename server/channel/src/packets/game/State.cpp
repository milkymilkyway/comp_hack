...
static void SendStateData(
  ...
  tokuseiManager->SendCostAdjustments(cState->GetEntityID(), client);
  ...
  if (!channelSwitched) {
    ...
    if (!greetMsg.IsEmpty()) {
      chatManager->SendChatMessage(client, ChatType_t::CHAT_SELF, greetMsg);
    }
  ...
  for (auto post : objects::PostItem::LoadPostItemListByAccount(
           server->GetLobbyDatabase(), state->GetAccountUID())) {
    ...
    if (dgState) {
      ...
      for (uint32_t statusID : SVR_CONST.STATUS_DIGITALIZE) {
        ...
        if (timeLeft) {
          ...
          p.WriteS64Little(state->GetObjectID(dgState->GetDemon().GetUUID()));
