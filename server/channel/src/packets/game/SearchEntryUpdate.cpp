...
bool Parsers::SearchEntryUpdate::Parse(
  ...
  if (!existing) {
    LogGeneralError([&]() {
      return libcomp::String(
                 "SearchEntryUpdate with invalid entry ID encountered: %1\n")
          .Arg(type);
    });
  } else if (existing->GetSourceCID() != state->GetWorldCID()) {
    LogGeneralError([&]() {
      return libcomp::String(
                 "SearchEntryUpdate request encountered with an entry ID "
  ...
  if (success) {
    ...
    switch ((objects::SearchEntry::Type_t)type) {
      ...
      case objects::SearchEntry::Type_t::FREE_RECRUIT:
        ...
        LogGeneralError([&]() {
          return libcomp::String(
                     "Unsupported SearchEntryUpdate type encountered: %1\n")
              .Arg(type);
        });
      ...
      default:
        LogGeneralError([&]() {
          return libcomp::String(
                     "Invalid SearchEntryUpdate type encountered: %1\n")
              .Arg(type);
        });
    ...
    if (success) {
      ...
      LogGeneralError([&]() {
        return libcomp::String(
                   "Invalid SearchEntryUpdate request encountered: %1\n")
            .Arg(type);
      });
    }
  }
