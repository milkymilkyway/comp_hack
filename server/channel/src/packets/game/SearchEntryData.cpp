...
bool Parsers::SearchEntryData::Parse(
  ...
  if (entry) {
    ...
    switch ((objects::SearchEntry::Type_t)type) {
      ...
      case objects::SearchEntry::Type_t::CLAN_JOIN: {
        ...
        auto cLogin =
            server->GetAccountManager()->GetActiveLogin(entry->GetRelatedTo());
      ...
      case objects::SearchEntry::Type_t::CLAN_RECRUIT: {
        ...
        if (clan) {
          ...
          for (libobjgen::UUID& uuid : memberUIDs) {
            auto stats =
                objects::EntityStats::LoadEntityStatsByEntity(worldDB, uuid);
            if (stats) {
              memberCount++;
        ...
        auto cLogin = masterMember
                          ? server->GetAccountManager()->GetActiveLogin(
                                masterMember->GetCharacter())
                          : nullptr;
        reply.WriteS8(cLogin ? (int8_t)cLogin->GetStatus() : 0);
