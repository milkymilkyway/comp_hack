...
bool Parsers::ShopData::Parse(
  ...
  if (cEvent) {
    ...
    for (uint8_t i = 0; i < (uint8_t)shopData->TabsCount(); i++) {
      ...
      if (tab->ConditionsCount() > 0 &&
          !eventManager->EvaluateEventConditions(zone, tab->GetConditions(),
                                                 client)) {
  ...
  for (uint8_t i = 0; i < (uint8_t)shopData->TabsCount(); i++) {
    ...
    for (auto product : products) {
      ...
      bool moonRestricted =
          product->GetMoonRestrict() && product->GetMoonRestrict() != 0xFFFF;
      if (moonRestricted) {
        flags |= 0x40;
      }
      ...
      if (it == productTrends.end()) {
        ...
        auto pData = definitionManager->GetShopProductData(
            (uint32_t)product->GetProductID());
        auto def =
            pData ? definitionManager->GetItemData(pData->GetItem()) : nullptr;
        ...
        productTrends[product->GetProductID()] = std::make_pair(trend, price);
      } else {
