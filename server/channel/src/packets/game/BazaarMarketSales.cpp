...
bool Parsers::BazaarMarketSales::Parse(
  ...
  auto bItems = objects::BazaarItem::LoadBazaarItemListByAccount(
      worldDB, state->GetAccountUID());
  ...
  if (bItem && amount > 0) {
    ...
    if (success) {
      ...
      if (!server->GetWorldDatabase()->ProcessChangeSet(dbChanges)) {
        LogBazaarError([&]() {
          return libcomp::String("BazaarMarketSales failed to save: %1\n")
  ...
  reply.WritePacketCode(
      ChannelToClientPacketCode_t::PACKET_BAZAAR_MARKET_SALES);
