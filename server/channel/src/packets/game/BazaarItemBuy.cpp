...
bool Parsers::BazaarItemBuy::Parse(
  ...
  if (bState && item) {
    ...
    if (!bItem || totalMacca < (uint64_t)bItem->GetCost()) {
      ...
      errorCode = -2;
                                            ...
                                            character, SVR_CONST.ITEM_MACCA,
                                            inventory) < bItem->GetCost()) {
      ...
      LogBazaarError([accountUID]() {
        return libcomp::String(
                   "BazaarItemBuy failed due to required macca splitting "
                   "without enough space available: %1\n")
      ...
      if (!characterManager->PayMacca(client, cost)) {
        ...
        if (!inventory->GetItems((size_t)destSlot).IsNull()) {
          for (size_t freeSlot :
               characterManager->GetFreeSlots(client, inventory)) {
        ...
        if (!server->GetWorldDatabase()->ProcessChangeSet(dbChanges)) {
          auto accountUID = state->GetAccountUID();
          LogBazaarError([accountUID]() {
            return libcomp::String("BazaarItemBuy failed to save: %1\n")
                .Arg(accountUID.ToString());
          });
        ...
        LogBazaarDebug([item, bItem, cost, ownerUID, accountUID]() {
          return libcomp::String(
                     "Item %1 (type %2) purchased for %3 macca from player %4 "
                     "by player: %5\n")
              .Arg(item->GetUUID().ToString())
              .Arg(item->GetType())
              .Arg(cost)
              .Arg(ownerUID.ToString())
              .Arg(accountUID.ToString());
        });
      }
    }
