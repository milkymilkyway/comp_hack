...
bool Parsers::AppearanceAlter::Parse(
  ...
  bool success =
      itemData && inventory && item->GetItemBox() == inventory->GetUUID();
  if (success) {
    ...
    if (appearanceID != 0) {
        ...
        default:
          LogGeneralError([&]() {
            return libcomp::String(
                       "Request to alter appearance received for an invalid "
                       "appearance item: %1\n")
                .Arg(item->GetType());
          });
      ...
      LogGeneralError([&]() {
        return libcomp::String(
                   "Request to alter appearance received for an item with no "
                   "appearance alteration value: %1\n")
            .Arg(item->GetType());
      });
  ...
  reply.WritePacketCode(ChannelToClientPacketCode_t::PACKET_APPEARANCE_ALTER);
