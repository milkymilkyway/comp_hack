...
void SendDemonQuestData(const std::shared_ptr<ChannelServer> server,
  ...
  reply.WritePacketCode(ChannelToClientPacketCode_t::PACKET_DEMON_QUEST_DATA);
  if (dQuest && character->GetDemonQuest().IsNull()) {
    ...
    int8_t bonusCount =
        (int8_t)(dQuest->BonusItemsCount() + dQuest->BonusTitlesCount() +
                 dQuest->BonusXPCount());
...
bool Parsers::DemonQuestData::Parse(
