...
bool Parsers::SearchAppReply::Parse(
        ...
        if (actionType == 0) {
          auto target =
              libcomp::PersistentObject::LoadObjectByUUID<objects::Character>(
                  worldDB, replyEntry->GetRelatedTo());
          ...
          if (target) {
            ...
            request.WriteString16Little(
                libcomp::Convert::Encoding_t::ENCODING_UTF8, target->GetName(),
                true);
            request.WriteU32Little(
                0);  // Unknown party ID, let the world decide what to do
      ...
      case objects::SearchEntry::Type_t::PARTY_RECRUIT:
        if (actionType == 0) {
          ...
          if (target) {
            ...
            request.WritePacketCode(InternalPacketCode_t::PACKET_PARTY_UPDATE);
            request.WriteU8(
                (int8_t)InternalPacketAction_t::PACKET_ACTION_YN_REQUEST);
            request.WriteU8(0);  // Not from (normal) recruit
            member->SavePacket(request, false);
            request.WriteString16Little(
                libcomp::Convert::Encoding_t::ENCODING_UTF8, target->GetName(),
      ...
      default:
        LogGeneralError([&]() {
          return libcomp::String(
                     "Invalid SearchAppReply type encountered: %1\n")
              .Arg(parentType);
        });
  ...
  reply.WritePacketCode(
      ChannelToClientPacketCode_t::PACKET_SEARCH_APPLICATION_REPLY);
