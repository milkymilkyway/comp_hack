...
bool Parsers::ItemPromo::Parse(
  ...
  for (auto promo : objects::Promo::LoadPromoListByCode(db, code)) {
    if (promo->GetStartTime() <= now &&
        (!promo->GetEndTime() || promo->GetEndTime() >= now)) {
      bool valid = true;
      if (promo->GetUseLimit()) {
        auto exchanges = existing;
        exchanges.remove_if(
            [promo](const std::shared_ptr<objects::PromoExchange>& e) {
        ...
        switch (promo->GetLimitType()) {
          case objects::Promo::LimitType_t::PER_CHARACTER:
            exchanges.remove_if(
                [cState](const std::shared_ptr<objects::PromoExchange>& e) {
                  return e->GetCharacter().GetUUID() != cState->GetEntityUUID();
                });
            break;
          case objects::Promo::LimitType_t::PER_WORLD:
            exchanges.remove_if(
                [worldID](const std::shared_ptr<objects::PromoExchange>& e) {
      ...
      if (valid) {
        auto exchange =
            libcomp::PersistentObject::New<objects::PromoExchange>(true);
