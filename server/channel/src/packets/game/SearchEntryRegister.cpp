...
bool Parsers::SearchEntryRegister::Parse(
    ...
    if (existing->GetType() != entry->GetType()) {
      LogGeneralError([&]() {
        return libcomp::String(
                   "SearchEntryRegister request encountered while conflicting "
                   "entry of a different type exists: %1\n")
            .Arg(type);
      });
    ...
    switch ((objects::SearchEntry::Type_t)type) {
      ...
      case objects::SearchEntry::Type_t::CLAN_RECRUIT:
        if (p.Left() >= 13) {
          ...
          if (!clan) {
            LogGeneralErrorMsg(
                "SearchEntryRegister request encountered for clan recruitment "
                "when the requestor is not in a clan\n");
          ...
          entry->SetRelatedTo(
              state->GetCharacterState()->GetEntity()->GetClan().GetUUID());
      ...
      case objects::SearchEntry::Type_t::TRADE_SELLING:
        if (p.Left() >= 46) {
          ...
          auto itemData =
              server->GetDefinitionManager()->GetItemData((uint32_t)itemType);
          if (!itemData) {
      ...
      case objects::SearchEntry::Type_t::TRADE_BUYING_APP:
        if (p.Left() >= 6) {
          ...
          if (existing) {
            LogGeneralErrorMsg(
                "SearchEntryRegister request encountered for application with "
                "duplicate parent ID\n");
          ...
          if (parent) {
            ...
            LogGeneralErrorMsg(
                "SearchEntryRegister request encountered for an application to "
                "an invalid parent ID\n");
          }
        }
        break;
      default:
        LogGeneralErrorMsg(
            libcomp::String(
                "Invalid SearchEntryRegister type encountered: %1\n")
                .Arg(type));
  ...
  if (success) {
    ...
    LogGeneralErrorMsg(
        libcomp::String("Invalid SearchEntryRegister request encountered: %1\n")
            .Arg(type));
  }
