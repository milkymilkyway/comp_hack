...
bool Parsers::ItemPrice::Parse(
  ...
  if (itemData) {
    ...
    if (itemData->GetBasic()->GetEquipType() !=
      ...
      if (((int32_t)basicEffect != -1) || ((int32_t)specialEffect != -1)) {
      ...
      int8_t itemDurMax = (int8_t)itemData->GetPossession()->GetDurability();
      ...
      if ((int8_t)(durability / 1000) != maxDurability) {
        ...
        price =
            (int32_t)(price - (maxDurability - (int8_t)(durability / 1000)) *
                                  itemData->GetBasic()->GetRepairPrice() * 2);
      }
    }
