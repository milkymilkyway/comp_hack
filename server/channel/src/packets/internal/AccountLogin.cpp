...
bool Parsers::AccountLogin::Parse(
  ...
  if (0 == p.Left()) {
    LogAccountManagerErrorMsg("Invalid response received for AccountLogin.\n");
  ...
  if (responseCode == 1) {
    ...
    if (p.ReadU8() == 1) {
      ...
      if (!channelLogin->LoadPacket(p, false)) {
        LogAccountManagerErrorMsg(
            "Invalid ChannelLogin response received for AccountLogin.\n");
    ...
    if (nullptr == account) {
      LogAccountManagerErrorMsg(
          "Unknown account returned from AccountLogin response.\n");
    ...
    auto character =
        response.GetCharacterLogin() != nullptr
            ? response.GetCharacterLogin()->GetCharacter().Get(worldDB, true)
            : nullptr;
    if (nullptr == character) {
      LogAccountManagerErrorMsg(
          "Invalid character returned from AccountLogin response.\n");
    ...
    auto character =
        charLogin->GetCharacter().Get(server->GetWorldDatabase(), true);
    if (character) {
      uint32_t zoneID = 0, dynamicMapID = 0;
      int8_t channelID = -1;
      float x = 0.f, y = 0.f, rot = 0.f;
      if (server->GetZoneManager()->GetLoginZone(
              character, zoneID, dynamicMapID, channelID, x, y, rot)) {
    ...
    server->GetManagerConnection()->GetWorldConnection()->SendPacket(reply);
  } else {
