...
void QueuePartyMemberInfo(std::shared_ptr<ChannelClientConnection> client,
  ...
  auto partyState =
      ClientState::GetEntityClientState(member->GetWorldCID(), true);
...
bool Parsers::PartyUpdate::Parse(
  ...
  switch ((InternalPacketAction_t)mode) {
    ...
    case InternalPacketAction_t::PACKET_ACTION_UPDATE: {
      ...
      auto party = exists ? std::make_shared<objects::Party>() : nullptr;
      ...
      for (auto client : clients) {
        ...
        if (party && party->MemberIDsContains(state->GetWorldCID())) {
          ...
          // Removing
          state->GetAccountLogin()->GetCharacterLogin()->SetPartyID(0);
      ...
      for (auto client : clients) {
        ...
        if (recalcEntities.find(cState->GetEntityID()) ==
            recalcEntities.end()) {
    ...
    case InternalPacketAction_t::PACKET_ACTION_GROUP_LEADER_UPDATE: {
      ...
      if (!isResponse) {
        ...
        request.WritePacketCode(
            ChannelToClientPacketCode_t::PACKET_PARTY_LEADER_UPDATED);
        request.WriteS32Little(localEntityID);
