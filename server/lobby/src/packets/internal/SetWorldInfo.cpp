...
bool SetWorldInfoFromPacket(
  ...
  auto svr =
      objects::RegisteredWorld::LoadRegisteredWorldByID(mainDB, p.ReadU8());
  ...
  if (!dbConfig->LoadPacket(p, false)) {
    LogGeneralCriticalMsg(
        "World Server did not supply a valid database connection configuration "
  ...
  connection->SetName(
      libcomp::String("world:%1:%2").Arg(svr->GetID()).Arg(svr->GetName()));
  ...
  if (std::dynamic_pointer_cast<objects::LobbyConfig>(server->GetConfig())
    ...
    server->GetAccountManager()->DeleteKillTimeExceededCharacters(svr->GetID());
  }
