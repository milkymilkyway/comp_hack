...
ManagerConnection::ManagerConnection(
    std::weak_ptr<libcomp::BaseServer> server, asio::io_service* service,
...
bool ManagerConnection::ProcessMessage(
  ...
  switch (cMessage->GetConnectionMessageType()) {
    case libcomp::Message::ConnectionMessageType::
      ...
      const libcomp::Message::WorldNotification* notification =
          dynamic_cast<const libcomp::Message::WorldNotification*>(cMessage);

      uint16_t port = notification->GetPort();
      libcomp::String address = notification->GetAddress();

      LogConnectionDebug([&]() {
        return libcomp::String("Attempting to connect back to World: %1:%2\n")
            .Arg(address)
            .Arg(port);
      });
      ...
      if (worldConnection->Connect(address, port, true)) {
        ...
        LogConnectionInfo([&]() {
          return libcomp::String("New World connection established: %1:%2\n")
              .Arg(address)
              .Arg(port);
        });
    ...
    case libcomp::Message::ConnectionMessageType::
      ...
      auto world = GetWorldByConnection(
    ...
    case libcomp::Message::ConnectionMessageType::
      ...
      auto clientConnection =
          std::dynamic_pointer_cast<lobby::LobbyClientConnection>(connection);
...
bool ManagerConnection::InitializeWorld(
    const std::shared_ptr<lobby::World>& world) {
...
std::shared_ptr<lobby::World> ManagerConnection::GetWorldByID(
    uint8_t id) const {
    ...
    if (nullptr != connection) {
      if (nullptr == existing->GetConnection()) {
        ...
        LogConnectionCriticalMsg(
            "Multiple world servers registered with the same information.\n");
...
void ManagerConnection::RemoveWorld(std::shared_ptr<lobby::World>& world) {
  if (nullptr != world) {
    ...
    if (iter != mWorlds.end()) {
      ...
      if (nullptr != svr) {
        LogConnectionInfo([&]() {
          return libcomp::String("World connection removed: (%1) %2\n")
              .Arg(svr->GetID())
              .Arg(svr->GetName());
        });
    ...
    iter = std::find(mUnregisteredWorlds.begin(), mUnregisteredWorlds.end(),
                     world);
...
const std::shared_ptr<LobbyClientConnection>
ManagerConnection::GetClientConnection(const libcomp::String& username) const {
...
void ManagerConnection::SetClientConnection(
    const std::shared_ptr<LobbyClientConnection>& connection) {
...
void ManagerConnection::RemoveClientConnection(
    const std::shared_ptr<LobbyClientConnection>& connection) {
  if (nullptr == connection) {
    return;
  }
