...
ErrorCodes_t AccountManager::WebAuthLogin(const libcomp::String& username,
  ...
  LogAccountManagerDebug([&]() {
    return libcomp::String(
               "Attempting to perform a web auth login for account '%1'.\n")
        .Arg(username);
  });
  ...
  if (!mServer) {
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Web auth login for account '%1' failed with a system "
                 "error.\n")
          .Arg(username);
    });
  ...
  auto config =
      std::dynamic_pointer_cast<objects::LobbyConfig>(mServer->GetConfig());
  ...
  uint32_t requiredClientVersion =
      static_cast<uint32_t>(config->GetClientVersion() * 1000.0f + 0.5f);

  // Check the client version first.
  if (requiredClientVersion != clientVersion) {
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Web auth login for account '%1' failed with a wrong client "
                 "version. Expected version %2.%3 but got version %4.%5.\n")
          .Arg(username)
          .Arg(requiredClientVersion / 1000)
          .Arg(requiredClientVersion % 1000)
          .Arg(clientVersion / 1000)
          .Arg(clientVersion % 1000);
    });
  ...
  if (!account) {
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Web auth login for account '%1' failed with a bad username "
  ...
  if (checkPassword) {
    ...
    if (account->GetPassword() !=
        libcomp::Crypto::HashPassword(password, account->GetSalt())) {
      LogAccountManagerDebug([&]() {
        return libcomp::String(
                   "Web auth login for account '%1' failed with a bad "
                   "password.\n")
            .Arg(username);
      });
  ...
  if (objects::AccountLogin::State_t::OFFLINE != state &&
    ...
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Web auth login for account '%1' failed because it is already "
                 "online.\n")
          .Arg(username);
    });
  ...
  if (account->GetAPIOnly()) {
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Web auth login for API only account '%1' failed.\n")
          .Arg(username);
    });
  ...
  mServer->GetTimerManager()->ScheduleEventIn(
      static_cast<int>(config->GetWebAuthTimeOut()),
      [this](const libcomp::String& _username, const libcomp::String& _sid) {
  ...
  LogAccountManagerDebug([&]() {
    return libcomp::String(
               "Web auth login for account '%1' has now passed web "
               "authentication.\n")
        .Arg(username);
  });
...
ErrorCodes_t AccountManager::WebAuthLoginApi(const libcomp::String& username,
                                             uint32_t clientVersion,
                                             libcomp::String& sid) {
  return WebAuthLogin(username, libcomp::String(), clientVersion, sid, false);
}

ErrorCodes_t AccountManager::LobbyLogin(const libcomp::String& username,
                                        const libcomp::String& sid,
                                        libcomp::String& sid2, int maxClients) {
  LogAccountManagerDebug([&]() {
    return libcomp::String(
               "Attempting to perform a login with SID for account '%1'.\n")
        .Arg(username);
  });
  ...
  if (!login) {
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Login (via web auth) for account '%1' failed with a system "
                 "error.\n")
          .Arg(username);
    });
  ...
  if (sid != login->GetSessionID()) {
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Login (via web auth) for account '%1' failed because it did "
                 "not provide a correct SID.\n")
          .Arg(username);
    });
  ...
  if (0 < maxClients && login->GetMachineUUID().IsEmpty()) {
    LogAccountManagerInfo([&]() {
      return libcomp::String(
                 "Classic login for account '%1' failed due to multiclienting "
                 "(not using classic login).\n")
          .Arg(username);
    });
...
ErrorCodes_t AccountManager::LobbyLogin(const libcomp::String& username,
  ...
  LogAccountManagerDebug([&]() {
    return libcomp::String(
               "Attempting to perform a classic login for account '%1'.\n")
        .Arg(username);
  });
  ...
  if (!account) {
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Classic login for account '%1' failed with a bad username "
                 "(no account data found).\n")
          .Arg(username);
    });
  ...
  if (objects::AccountLogin::State_t::OFFLINE != state &&
    ...
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Classic login for account '%1' failed because it is already "
                 "online.\n")
          .Arg(username);
    });
  ...
  if (0 < maxClients && login->GetMachineUUID() != machineUUID) {
    if (!login->GetMachineUUID().IsEmpty()) {
      LogAccountManagerDebug([&]() {
        return libcomp::String(
                   "Account '%1' login switching from one client machine to "
                   "another.\n")
            .Arg(username);
      });
    ...
    if (matchIt != mMachineUUIDs.end()) {
      ...
      if (maxClients <= matchIt->second) {
        LogAccountManagerDebug([username, machineUUID]() {
          return libcomp::String(
                     "Classic login for account '%1' failed due to "
                     "multiclienting (machine UUID: %2).\n")
              .Arg(username)
              .Arg(machineUUID);
        });
      ...
      LogAccountManagerDebug([machineUUID, username, matchIt]() {
        return libcomp::String(
                   "Machine UUID '%1' from account '%2' login raised to %3.\n")
            .Arg(machineUUID)
            .Arg(username)
      ...
      LogAccountManagerDebug([machineUUID, username]() {
        return libcomp::String(
                   "Machine UUID '%1' from account '%2' login raised to 1.\n")
            .Arg(machineUUID)
            .Arg(username);
      });
...
std::shared_ptr<objects::AccountLogin> AccountManager::StartChannelLogin(
  ...
  if (!login) {
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Login to channel for account '%1' failed with a system "
                 "error.\n")
          .Arg(username);
    });
  ...
  if (objects::AccountLogin::State_t::LOBBY != login->GetState()) {
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Login to channel for account '%1' failed because it is not "
                 "in the lobby state.\n")
          .Arg(username);
    });
...
ErrorCodes_t AccountManager::SwitchToChannel(const libcomp::String& username,
  ...
  LogAccountManagerDebug([&]() {
    return libcomp::String(
               "Attempting to perform a login to channel %1 on world %2 for "
               "account '%3'.\n")
        .Arg(channelID)
        .Arg(worldID)
        .Arg(username);
...
ErrorCodes_t AccountManager::CompleteChannelLogin(
  ...
  LogAccountManagerDebug([&]() {
    return libcomp::String(
               "Attempting to complete a login to channel %1 on world %2 for "
               "account '%3'.\n")
        .Arg(channelID)
        .Arg(worldID)
        .Arg(username);
  ...
  if (objects::AccountLogin::State_t::LOBBY_TO_CHANNEL != login->GetState() &&
    ...
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Login to channel for account '%1' failed because it is not "
                 "in the lobby to channel or channel to channel state.\n")
          .Arg(username);
    });
  ...
  if (cLogin->GetWorldID() != worldID || cLogin->GetChannelID() != channelID) {
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Login to channel for account '%1' failed because the "
                 "completion is for a different world or channel.\n")
          .Arg(username);
    });
...
bool AccountManager::ChannelToChannelSwitch(const libcomp::String& username,
  ...
  if (!login) {
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Channel to channel switch for account '%1' failed with a "
                 "system error.\n")
          .Arg(username);
    });
  ...
  if (!cLogin || objects::AccountLogin::State_t::CHANNEL != login->GetState()) {
    LogAccountManagerDebug([&]() {
      return libcomp::String(
                 "Channel to channel for account '%1' failed because it is not "
                 "in the channel state with a valid character.\n")
          .Arg(username);
    });
...
bool AccountManager::Logout(const libcomp::String& username) {
  ...
  if (objects::AccountLogin::State_t::LOBBY == login->GetState()) {
    ...
    LogAccountManagerDebug([username, config]() {
      return libcomp::String(
                 "Account session for '%1' will expire in %2 second(s).\n")
          .Arg(username)
...
void AccountManager::ExpireSession(const libcomp::String& username,
  ...
  if (mAccountMap.end() != pair) {
    ...
    if (account &&
        objects::AccountLogin::State_t::LOBBY_WAIT == account->GetState() &&
    ...
    if (entryIt != mMachineUUIDs.end()) {
      if (1 < entryIt->second) {
        LogAccountManagerDebug([machineUUID, username, entryIt]() {
          return libcomp::String(
                     "Machine UUID '%1' from account '%2' login lowered to "
                     "%3.\n")
              .Arg(machineUUID)
              .Arg(username)
  ...
  if (character) {
    ...
    if (character->GetKillTime() > 0) {
      ...
      if (deleteMinutes > 0) {
        // Set the kill time
        time_t now = time(0);
        character->SetKillTime(
            static_cast<uint32_t>(now + (deleteMinutes * 60)));
      } else {
...
bool AccountManager::DeleteKillTimeExceededCharacters(uint8_t worldID) {
  ...
  LogAccountManagerDebug([&]() {
    return libcomp::String(
               "Loading kill time exceeded characters for world server: (%1) "
               "%2\n")
        .Arg(svr->GetID())
        .Arg(svr->GetName());
  });
  ...
  for (auto character :
       libcomp::PersistentObject::LoadAll<objects::Character>(worldDB)) {
  ...
  if (exceeded.size() > 0) {
    ...
    while (exceeded.size() > 0) {
      ...
      exceeded.remove_if(
          [accountUID, &subset](const std::shared_ptr<objects::Character>& c) {
      ...
      if (account) {
        ...
        LogAccountManagerDebug([&]() {
          return libcomp::String(
                     "Failed to load account %1 associated to kill time "
                     "exceeded character(s)\n")
...
bool AccountManager::SetCharacterOnAccount(
  ...
  if (nextCID == MAX_CHARACTER) {
    LogAccountManagerError([&]() {
      return libcomp::String("Character failed to be created on account: %1\n")
  ...
  if (!account->SetCharacters(nextCID, character) ||
    ...
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "Account character array failed to save for account %1\n")
          .Arg(account->GetUUID().ToString());
    });
...
AccountManager::GetCharactersForDeletion(
    const std::shared_ptr<objects::Account>& account) {
...
bool AccountManager::DeleteCharacter(
  ...
  if (cid == MAX_CHARACTER) {
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "Attempted to delete a character no longer associated to its "
                 "parent account: %1\n")
  ...
  if (!account->Update(mServer->GetMainDatabase())) {
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "Account failed to update after character deletion: %1\n")
          .Arg(character->GetUUID().ToString());
    });
...
std::shared_ptr<WebGameApiSession> AccountManager::GetWebGameApiSession(
  ...
  if (it != mWebGameSessions.end()) {
    ...
    if (gameSession->GetSessionID() == sessionID) {
      ...
      if (it2 != mWebGameAPISessions.end()) {
        if (it2->second->clientAddress != clientAddress) {
          LogAccountManagerError([&]() {
            return libcomp::String(
                       "Second web-game session attempted for account: %1\n")
                .Arg(username);
          });
    ...
    LogAccountManagerError([&]() {
      return libcomp::String(
                 "Web-game API session requested from account with no active "
                 "web-game session: %1\n")
          .Arg(username);
    });
  }
