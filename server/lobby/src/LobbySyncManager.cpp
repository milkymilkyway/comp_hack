...
bool LobbySyncManager::Initialize() {
  ...
  // Build the configs
  auto cfg = std::make_shared<ObjectConfig>("Account", true, lobbyDB);
  cfg->UpdateHandler =
      &DataSyncManager::Update<LobbySyncManager, objects::Account>;
  cfg->DynamicHandler = true;

  mRegisteredTypes["Account"] = cfg;

  cfg = std::make_shared<ObjectConfig>("Character", false, nullptr);
  cfg->UpdateHandler =
      &DataSyncManager::Update<LobbySyncManager, objects::Character>;
  cfg->DynamicHandler = true;

  mRegisteredTypes["Character"] = cfg;

  cfg = std::make_shared<ObjectConfig>("CharacterProgress", false, nullptr);
  cfg->UpdateHandler =
      &DataSyncManager::Update<LobbySyncManager, objects::CharacterProgress>;
  cfg->DynamicHandler = true;
...
namespace lobby {
template <>
int8_t LobbySyncManager::Update<objects::Account>(
...
template <>
int8_t LobbySyncManager::Update<objects::Character>(
    const libcomp::String& type, const std::shared_ptr<libcomp::Object>& obj,
    bool isRemove, const libcomp::String& source) {
  (void)type;
  (void)source;
...
void LobbySyncManager::SyncAccount(
  ...
  if (worldID >= 0) {
    auto world = server->GetManagerConnection()->GetWorldByID((uint8_t)worldID);
    if (world) {
      libcomp::Packet p;
...
void LobbySyncManager::SyncCharacter(
    const std::shared_ptr<objects::Character>& character, bool isRemove) {
  auto server = mServer.lock();

  uint8_t worldID = character->GetWorldID();

  auto world = server->GetManagerConnection()->GetWorldByID(worldID);
  if (world) {
...
void LobbySyncManager::SyncCharacterProgress(
  ...
  if (character) {
    ...
    LogDataSyncManagerDebug([&]() {
      return libcomp::String(
                 "CharacterProgress failed to sync for character: %1\n")
