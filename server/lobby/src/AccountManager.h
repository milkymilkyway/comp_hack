...
class AccountManager {
 public:
  ...
  /**
   * Check if an account is enabled (not banned/disabled)
   * @param account Pointer to the account to check the ban on.
   * @return True is account is enabled, false if it is not.
   */
  bool IsAccountEnabled(const std::shared_ptr<objects::Account>& account);
   ...
   * @note This function is thread safe.
   */
  ErrorCodes_t SwitchToChannel(const libcomp::String& username, int8_t worldID,
                               int8_t channelID);
   ...
   * @note This function is thread safe.
   */
  bool ChannelToChannelSwitch(const libcomp::String& username, int8_t channelID,
                              uint32_t sessionKey);
   ...
   */
  bool IsLoggedIn(const libcomp::String& username, int8_t& world);
   ...
   */
  std::list<libcomp::String> GetUsersInWorld(int8_t world, int8_t channel = -1);
   ...
   */
  bool UpdateKillTime(const libcomp::String& username, uint8_t cid,
 ...
 private:
  ...
  std::unordered_map<libcomp::String, std::shared_ptr<objects::AccountLogin>>
      mAccountMap;
  ...
  std::unordered_map<libcomp::String, std::shared_ptr<objects::WebGameSession>>
      mWebGameSessions;

  /// List of web-game API sessions by username. Only valid for as long
  /// as there is a web-game session active
  std::unordered_map<libcomp::String, std::shared_ptr<WebGameApiSession>>
      mWebGameAPISessions;
