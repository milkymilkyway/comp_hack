...
void HttpConnection::SetMessageQueue(
...
void HttpConnection::PacketReceived(libcomp::Packet& packet) {
  ...
  if (std::string::npos != headerEnd) {
    ...
    if (std::regex_search(mRequest, match, contentExpr)) {
      ...
      size_t contentLength =
          libcomp::String("%1").Arg(match.str(1)).ToInteger<size_t>(&ok);
  ...
  requestPacket.WriteArray(request.c_str(),
  ...
  if (this == self.get() && nullptr != mMessageQueue) {
    ...
    mMessageQueue->Enqueue(new libcomp::Message::Packet(self, 0x0000u, copy));
  }
}
